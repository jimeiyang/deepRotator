...
class NetworkBuilder(object):
  ...
  def _build_rnn_network(self, wtype='xavier', std=0.01, batchsize=1, numstep=24):
    ...
    drop1_id = self._make_dropout_layer(network)
    ...
    drop1_view = self._make_dropout_layer(network)
    ...
    drop2_view.append(self._make_dropout_layer(network))
    ...
    connections.append((fc1.name, (fc1.top, relu4.bottom, relu4.top, drop4.bottom, drop4.top, fc2.bottom)))
    connections.append((fc2.name, (fc2.top, relu5.bottom)))
    connections.append((relu5.name, (relu5.top, drop5.bottom, drop5.top, enc_split.bottom)))
    connections.append((enc_split.name, (enc_split.top, fc1_id.bottom)))
    connections.append((fc1_id.name, (fc1_id.top, relu1_id.bottom, relu1_id.top, drop1_id.bottom, drop1_id.top, id_split.bottom)))
    connections.append((enc_split.name, (enc_split.top, fc1_view.bottom)))
    connections.append((fc1_view.name, (fc1_view.top, relu1_view.bottom, relu1_view.top, drop1_view.bottom, drop1_view.top, tensor_view[-1].bottom)))
    ...
    for t in range(numstep):
      ...
      dec_drop1.append(self._make_dropout_layer(network))
      ...
      dec_drop2.append(self._make_dropout_layer(network))
      ...
      dec_img_drop1.append(self._make_dropout_layer(network))
      ...
      dec_mask_drop1.append(self._make_dropout_layer(network))
      ...
      connections.append((relu2_view[-1].name, (relu2_view[-1].top, drop2_view[-1].bottom, drop2_view[-1].top, relu2_view_split[-1].bottom)))
      ...
      connections.append((dec_fc1[-1].name, (dec_fc1[-1].top, dec_relu1[-1].bottom, dec_relu1[-1].top, dec_drop1[-1].bottom, dec_drop1[-1].top, dec_fc2[-1].bottom)))
      connections.append((dec_fc2[-1].name, (dec_fc2[-1].top, dec_relu2[-1].bottom)))
      connections.append((dec_relu2[-1].name, (dec_relu2[-1].top, dec_drop2[-1].bottom, dec_drop2[-1].top, dec_relu2_split[-1].bottom)))
      # dec image connections.
      connections.append((dec_relu2_split[-1].name, (dec_relu2_split[-1].top, dec_img_fc1[-1].bottom)))
      connections.append((dec_img_fc1[-1].name, (dec_img_fc1[-1].top, dec_img_relu1[-1].bottom, dec_img_relu1[-1].top, dec_img_drop1[-1].bottom, dec_img_drop1[-1].top, dec_img_fold[-1].bottom)))
      ...
      connections.append((dec_mask_fc1[-1].name, (dec_mask_fc1[-1].top, dec_mask_relu1[-1].bottom, dec_mask_relu1[-1].top, dec_mask_drop1[-1].bottom, dec_mask_drop1[-1].top, dec_mask_fold[-1].bottom)))
    ...
    layers = [ conv1, relu1, conv2, relu2, conv3, relu3, fc1, relu4, drop4, fc2, relu5, drop5, enc_split, fc1_id, relu1_id, drop1_id, id_split ]
    layers += tensor_view
    layers += relu2_view
    layers += drop2_view
...
if __name__ == '__main__':
  ...
  __Network_builder__.build_network(netname='rnn', batchsize=1, numstep=16)
