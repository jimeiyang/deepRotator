...
UnpoolMethod = caffe_pb2.UnpoolingParameter.UnpoolMethod
...
class NetworkBuilder(object):
  ...
  def _make_unpool_layer(self, network, kernel_size, stride=1):
    """Make max unpooling layer."""

    layer = network.layers.add()
    layer.name = 'unpool_%dx%d_%d' % (kernel_size, kernel_size, stride)

    layer.type = LayerType.Value('UNPOOLING')
    params = layer.unpooling_param
    params.unpool = UnpoolMethod.Value('MAX')
    params.kernel_size = kernel_size
    params.stride = stride
  ...
  def _build_rnn_network(self, wtype='xavier', std=0.01, batchsize=100, numstep=24):
    ...
    conv1 = self._make_conv_layer(network, kernel_size=5, stride=1, pad=2, num_output=64, shared_name='conv1')
    conv1.bottom.append('images')
    relu1 = self._make_relu_layer(network)
    pool1 = self._make_maxpool_layer(network, kernel_size=2, stride=2)
    pool1.top.append('mask1')
    conv2 = self._make_conv_layer(network, kernel_size=5, stride=1, pad=2, num_output=128, shared_name='conv2')
    relu2 = self._make_relu_layer(network)
    pool2 = self._make_maxpool_layer(network, kernel_size=2, stride=2)
    pool2.top.append('mask2')
    conv3 = self._make_conv_layer(network, kernel_size=5, stride=1, pad=2, num_output=256, shared_name='conv3')
    relu3 = self._make_relu_layer(network)
    pool3 = self._make_maxpool_layer(network, kernel_size=2, stride=2)
    pool3.top.append('mask3')
    ...
    connections = []
    connections.append((conv1.name, (conv1.top, relu1.bottom, relu1.top, pool1.bottom)))
    connections.append((pool1.name, (pool1.top, conv2.bottom)))
    connections.append((conv2.name, (conv2.top, relu2.bottom, relu2.top, pool2.bottom)))
    connections.append((pool2.name, (pool2.top, conv3.bottom)))
    connections.append((conv3.name, (conv3.top, relu3.bottom, relu3.top, pool3.bottom)))
    connections.append((pool3.name, (pool3.top, fc1.bottom)))
    ...
    for t in range(numstep):
      ...
      dec_img_up1.append(self._make_unpool_layer(network, kernel_size=2, stride=2))
      ...
      dec_img_up2.append(self._make_unpool_layer(network, kernel_size=2, stride=2))
      ...
      dec_img_up3.append(self._make_unpool_layer(network, kernel_size=2, stride=2))
    ...
    tmp = reversed(pool1.top)
    pool1.ClearField('top')
    pool1.top.extend(tmp)

    for l in dec_img_up1[0:]:
      l.bottom.append('mask1')

    tmp = reversed(pool2.top)
    pool2.ClearField('top')
    pool2.top.extend(tmp)

    for l in dec_img_up2[0:]:
      l.bottom.append('mask2')

    tmp = reversed(pool3.top)
    pool3.ClearField('top')
    pool3.top.extend(tmp)

    for l in dec_img_up3[0:]:
      l.bottom.append('mask3')
  ...
  def build_network(self, netname, batchsize=1, numstep=2):
    """main method."""
...
if __name__ == '__main__':
  ...
  __Network_builder__.build_network(netname='rnn', batchsize=1, numstep=2)
